...
struct TP_QT_NO_EXPORT BaseChannelFileTransferType::Private {
    ...
    GetIODeviceCallback getIODeviceCB;
    ...
    TransferType transferType; // The value is valid only after AcceptFile or ProvideFile call.
...
QDateTime BaseChannelFileTransferType::Adaptee::date() const
{
    return mInterface->date();
}
        ...
        const Tp::Service::ChannelTypeFileTransferAdaptor::AcceptFileContextPtr &context)
{
    ...
    if (!mInterface->mPriv->getIODeviceCB.isValid()) {
    ...
    mInterface->mPriv->transferType = BaseChannelFileTransferType::Private::Incoming;
        ...
        const Tp::Service::ChannelTypeFileTransferAdaptor::ProvideFileContextPtr &context)
{
    ...
    mInterface->mPriv->transferType = BaseChannelFileTransferType::Private::Outgoing;

    DBusError error;
    mInterface->createSocket(addressType, accessControl, accessControlParam, &error);

    if (!error.isValid()) {
        mInterface->getIODevice(&error);
    }
 ...
 *
 * Default implementation support only IPv4 and IPv6 sockets with localhost access control.
 *
 * Usage:
 * Set getIODevice callback to a function with follow signature:
 * \code{.cpp}QIODevice *getIODevice(qulonglong offset, Tp::DBusError *error);\endcode
 *
 * The callback function should return the device, which would be used to get (if the channel is an incoming transfer)
 * or pass (for outgoing transfer) file content from/to underlying implementation. If the transfer is incoming, then
 * the method setInitialOffset() should be called with an actual offset of incoming data. (The requested offset can be
 * ignored for some reason, such as limitation of underlying protocol or implementation, or as result of negotiation
 * with remote side).
 *
 * If the transfer is outgoing, then connection manager should call remoteAcceptFile() with an offset, requested by the
 * remote side.
 *
 * Outgoing transfer process:
 * -# Client request a channel with ChannelType = TP_QT_IFACE_CHANNEL_TYPE_FILE_TRANSFER and other properties, such as
 *    Filename, Size and ContentType.
 * -# Connection manager creates a requested channel. The channel initial state is Pending.
 * -# Connection manager ask remote contact to accept the transfer.
 * -# Once contact accept file, connection manager call remoteAcceptFile() method. initialOffset setted to requested value
 *    without initialOffsetDefined signal. The channel state is Accepted now.
 * -# Client call ProvideFile method to configure the socket.
 * -# ProvideFile implementation (hidden in the interface private implementation):
 *     -# Call createSocket(). The called method should trigger (now or later) call to setClientSocket() to set a client socket.
 *     -# Call getIODevice() callback with an initial offset, requested by remote side.
 *        The callback must inform the interface about an actual offset via setInitialOffset() method.
 *        The callback should inform remote side about an actual data offset.
 *     -# Use socketAddress() method to return socket address.
 * -# After setClientSocket() call, the channel state becomes Open.
 * -# Client writes data to the socket.
 * -# The clientSocket emit readyRead() signal, the interface reads data from clientSocket and write it to device.
 * -# Device emit bytesWritten() signal, the interface updates transferredBytes count.
 * -# If transferredBytes == size, then the channel state changes to Completed.
 *    Else the interface waits for further data from the client socket.
 *
 * Incoming transfer process:
 * -# Connection manager creates not-requested channel with ChannelType = TP_QT_IFACE_CHANNEL_TYPE_FILE_TRANSFER and other
 *     properties, such as Filename, Size and ContentType. The channel initial state is Pending.
 * -# Client call AcceptFile method to configure the socket and request an initial offset
 * -# AcceptFile implementation (hidden in the interface private implementation):
 *     -# Call createSocket(). The called method should trigger (now or later) call to setClientSocket() to set a client socket.
 *     -# Set the initialOffset property to required by local client without initialOffsetDefined signal emission.
 *     -# Set channel state to Accepted.
 *     -# Use socketAddress() method to return socket address.
 * -# Connection manager respond on the state changes to accepted. It inform remote side about the requested offset.
 * -# Once connection manager got an actual initial offset from the remote side, it call setInitialOffset() method.
 * -# When initialOffset become defined, the interface call getIODevice() callback. The channel state becomes Open.
 * -# Connection manager writes data to the device.
 * -# The device emit bytesWritten() signal, the interface reads data from the device and write it to the clientSocket.
 * -# Client socket emit bytesWritten() signal, the interface updates transferredBytes count.
 * -# If transferredBytes == size, then the channel state changes to Completed.
 *    Else the interface waits for further data from the device socket.
 *
 * Subclassing:
 * + Reimplement a public virtual method availableSocketTypes() to expose extra socket types.
 * + Overload protected createSocket() method to provide own socket address type, access control and its param implementation.
 * + Custom createSocket() implementation MUST be accompanied with a custom socketAddress() method implementation.
 * + Use setClientSocket() method to pass a client socket (the method would set the state to Open).
 *
 */
...
BaseChannelFileTransferType::BaseChannelFileTransferType(const QString &contentType,
    ...
    : AbstractChannelInterface(TP_QT_IFACE_CHANNEL_TYPE_FILE_TRANSFER),
      mPriv(new Private(this, contentType, filename, size, contentHashType, contentHash, description, date))
{
}
...
void BaseChannelFileTransferType::writeAllAvailableBytes()
{
...
bool BaseChannelFileTransferType::getIODevice(Tp::DBusError *error)
{
    if (mPriv->device) {
        if (error) {
            error->set(QDBusError::errorString(QDBusError::InternalError), QLatin1String("Internal error"));
        }
        return false;
    }

    QIODevice *device = mPriv->getIODeviceCB(mPriv->initialOffset, error);

    QIODevice::OpenModeFlag flags = mPriv->transferType == Private::Incoming ? QIODevice::ReadOnly : QIODevice::WriteOnly;

    if (!device || (!device->isOpen() && !device->open(flags)) || !(device->openMode() & flags)) {
        if (error) {
            error->set(QDBusError::errorString(QDBusError::InternalError), QLatin1String("Internal error"));
        }
        return false;
...
/**
 * \fn void BaseChannelFileTransferType::setGetIODeviceCallback(const GetIODeviceCallback &cb)
 *
 * Setup GetIODevice method callback.
 *
 * If the file transfer is supposed to proceed, then the callback method MUST set initial offset,
 * The value, wanted by receiver, passed as the first callback argument.
 */
void BaseChannelFileTransferType::setGetIODeviceCallback(const GetIODeviceCallback &cb)
{
    mPriv->getIODeviceCB = cb;
}

void BaseChannelFileTransferType::remoteAcceptFile(qulonglong offset)
{
    mPriv->initialOffset = offset; // Do not use setInitialOffset() to avoid initialOffsetDefined() signal emission.
